...
class GoogleBase:
    ...
    SCOPE = 'your_scope'  # Remplacez par la portée appropriée

    def __init__(self):  # Utiliser __init__ à la place de _init
        self._location = None
    ...
    def _get_authorize_url(cls):
        query = {
            'scope': cls.SCOPE,
            'redirect_uri': 'https://votre-site-web.com/redirect-uri',  # Remplacez par votre URL de redirection
            'response_type': 'code',
    ...
    def _request(self, url, body=None, headers=None, retry_auth=True, method=None):
        ...
        request = requests.Request(url, data=body, headers=headers, method=method)
        try:
            response = requests.Session().send(request.prepare())

        except requests.HTTPError as e:
            if e.response.status_code == 401 and retry_auth:  # unauthorized, access token may have expired
                try:
    ...
    def _request_credentials(self, authorization_key):
        ...
        request = requests.Request(self.TOKEN_URL, data=body, headers=headers, method='POST')
        try:
            response = requests.Session().send(request.prepare())
...
class Dropbox(UploadService):
    ...
    def _request(self, url, body=None, headers=None, retry_auth=True):
        ...
        self.debug('requesting %s' % url)
        request = requests(url, data=body, headers=headers)
        try:
    ...
    def _request_credentials(self, authorization_key):
        ...
        request = requests(self.TOKEN_URL, data=body, headers=headers)
